...
<div class="card">
  <h5 class="card-header">Thay đổi đánh giá</h5>
  <div class="card-body">
    <form action="{{route('user.productreview.update',$review->id)}}" method="POST">
      ...
      <div class="form-group">
        ...
        <select name="status" id="" class="form-control">
          ...
          <option value="active" {{(($review->status=='active')? 'selected' : '')}}>Hiển thị</option>
          <option value="inactive" {{(($review->status=='inactive')? 'selected' : '')}}>Ẩn</option>
        </select>
      </div>
