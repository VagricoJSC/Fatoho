...
<div class="card">
  <h5 class="card-header">Thay đổi comment</h5>
  <div class="card-body">
    <form action="{{route('user.post-comment.update',$comment->id)}}" method="POST">
      @csrf
      @method('PATCH')
      <div class="form-group">
        <label for="name">Người comment:</label>
      ...
      <div class="form-group">
        ...
        <select name="status" id="" class="form-control">
          ...
          <option value="active" {{(($comment->status=='active')? 'selected' : '')}}>Hiển thị</option>
          <option value="inactive" {{(($comment->status=='inactive')? 'selected' : '')}}>Ẩn</option>
        </select>
      </div>
